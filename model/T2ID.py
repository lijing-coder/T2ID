...
class Base_Model(nn.Module):
    def __init__(self, num_classes=5, p_missing=0.4, memory_size=500, init_top_k=5, max_top_k=20, dimension=2048):
        ...
        # �ڴ��洢ÿ��ģ̬������
        #self.fundus_memory = deque(maxlen=memory_size)
        #self.oct_memory = deque(maxlen=memory_size)
        ...
        #self.fundus_branch = self._create_convnext_branch()
        #self.oct_branch = self.get_3d_resnet50()
        #self.oct_branch = torch.hub.load('facebookresearch/pytorchvideo', 'slow_r50', pretrained=True)
        #self.oct_branch.blocks[-1].pool = nn.Identity()   # ȥ������ avg pool
        #self.oct_branch.blocks[-1].proj = nn.Identity()   # ȥ�����ķ���ͷ
        
        # self.Regression_fundus = RegressionSubNetwork(dimension)
        # self.Regression_oct = RegressionSubNetwork(dimension)
        # self.Regression_fusion = RegressionSubNetwork(dimension*2)

        # self.Classification_fundus = ClassificationSubNetwork(dimension, num_classes)
        # self.Classification_oct = ClassificationSubNetwork(dimension, num_classes)
        # self.Classification_fusion = ClassificationSubNetwork(dimension*2, num_classes)
        ...
        #self.fc = nn.Linear(dimension*2, num_classes)
        #self.OCT_to_fundus_dim = nn.Linear(512, dimension)
    ...
    def _retrieve_cross_modal(self, query_feature, query_modality='fundus'):
        ...
        if final_similarities.numel() == 0:
            ...
            top5_indices = sorted_indices[:min(5, sorted_indices.numel())]
            top10_indices = sorted_indices[:min(10, sorted_indices.numel())]
            weighted_feature = target_set[closest_idx]
            debug_info = {
                "selected_indices": [int(closest_idx.item())],
                "selected_sims": [float(similarities[closest_idx].item())],
                "top1_index": int(closest_idx.item()),
                "top1_sim": float(similarities[closest_idx].item()),
                "top1_label": int(memory_labels[closest_idx].item()),
                "selected_labels": [int(memory_labels[closest_idx].item())],
                "top5_labels": [int(memory_labels[idx].item()) for idx in top5_indices.detach().cpu().tolist()],
                "top10_labels": [int(memory_labels[idx].item()) for idx in top10_indices.detach().cpu().tolist()]
            }
            return weighted_feature, debug_info
        ...
        top1_idx = torch.argmax(similarities)
        sorted_indices = torch.argsort(similarities, descending=True)
        top5_indices = sorted_indices[:min(5, sorted_indices.numel())]
        top10_indices = sorted_indices[:min(10, sorted_indices.numel())]
        selected_indices = final_indices.detach().cpu().tolist()
        debug_info = {
            "selected_indices": [int(i) for i in selected_indices],
            "selected_sims": [float(v) for v in final_similarities.detach().cpu().tolist()],
            "top1_index": int(top1_idx.item()),
            "top1_sim": float(similarities[top1_idx].item()),
            "top1_label": int(memory_labels[top1_idx].item()),
            "selected_labels": [int(memory_labels[idx].item()) for idx in selected_indices],
            "top5_labels": [int(memory_labels[idx].item()) for idx in top5_indices.detach().cpu().tolist()],
            "top10_labels": [int(memory_labels[idx].item()) for idx in top10_indices.detach().cpu().tolist()]
        }
    ...
    def forward(self, fundus_img, oct_img, label, update_memory=False):
        ...
        #模态手动缺失操作
        # batch_size = fundus_img.size(0)
        # num_missing = int(batch_size * self.p_missing)  # 总缺失样本数
        
        # # 初始化mask
        # missing_mask = torch.zeros(batch_size, 2).to(fundus_img.device)
        ...
        #     missing_mask[idx, random.randint(0, 1)] = 1        
        
        # 1. ����ȱʧ����
        # missing_mask = torch.zeros(batch_size, 2).to(fundus_img.device)
        # for i in range(batch_size):
        #     if random.random() < self.p_missing:
        #         missing_mask[i, random.randint(0, 1)] = 1
        ...
        # 4. ��ʼ����¼����
        # k_values = torch.zeros(batch_size).int().to(fundus_img.device)  # ��¼ÿ������ʹ�õ�kֵ
        # gate_decisions = torch.zeros(batch_size).to(fundus_img.device)  # ��¼�Ƿ�ȫ
        
        for i in range(batch_size):
            # ����fundusȱʧ�����
            if missing_mask[i, 0] == 1:
                gate_stats["missing_total"] += 1
                gate_stats["missing_fundus"] += 1
                existing_feature = oct_features[i]
                gate_val = self.gate_network_1(existing_feature)
                #print("gate_val:",gate_val)
                gate_decision = BinaryGate.apply(gate_val)
                #print("gate_decision:",gate_decision)
            
                if gate_decision > 0.5 and len(self.memory_bank) > 0:
                    gate_stats["gate_activated_total"] += 1
                    gate_stats["gate_activated_fundus"] += 1
                    filled_feature, retrieve_debug = self._retrieve_cross_modal(
                    ...
                    if retrieve_debug:
                        gate_stats["retrieval_events"] += 1
                        gate_stats["retrieved_k_sum"] += len(retrieve_debug["selected_indices"])
                        label_i = int(label[i].item())
                        if label_i == int(retrieve_debug["top1_label"]):
                            gate_stats["retrieval_label_match_top1"] += 1
                        if label_i in retrieve_debug.get("top5_labels", []):
                            gate_stats["retrieval_label_match_top5"] += 1
                        if label_i in retrieve_debug.get("top10_labels", []):
                            gate_stats["retrieval_label_match_top10"] += 1
                        if label_i in retrieve_debug["selected_labels"]:
                            gate_stats["retrieval_label_match_topk"] += 1
                    # gate_decisions[i] = 1
                    # k_values[i] = k_val
                else:
            ...
            # ����octȱʧ�����
                ...
                if gate_decision > 0.5 and len(self.memory_bank) > 0:
                    gate_stats["gate_activated_total"] += 1
                    gate_stats["gate_activated_oct"] += 1
                    filled_feature, retrieve_debug = self._retrieve_cross_modal(
        ...
        for i in range(batch_size):
            competition_records.append({
                ...
                "pred_clinic": int(pred_fundus[i].item()),
                "pred_derm": int(pred_oct[i].item()),
                "pred_fusion": int(pred_fusion[i].item()),
                "conf_clinic": float(TCPConfidence_fundus[i].view(-1)[0].item()),
                "conf_derm": float(TCPConfidence_oct[i].view(-1)[0].item()),
                "conf_fusion": float(Confidence_fusion[i].view(-1)[0].item())
            })
        ...
        #print(TCPLogit_fundus.shape, TCPLogit_oct.shape)
        #cts_loss = self.mine_loss_fn(TCPLogit_fundus, TCPLogit_oct)
        c_loss = (c_loss_fundus + c_loss_oct + c_loss_fusion)*w1

        #return Logit, feature, k_values, gate_decisions, c_loss, kd_loss
        return Logit_fusion, feature_fusion, c_loss, original_missing_features, reconstructed_features, TCPConfidence_fundus, TCPConfidence_oct, Confidence_fusion, gate_stats, competition_records
